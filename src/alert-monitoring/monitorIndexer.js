...
export class MonitorIndexer {
    static async monitoringIndexer() {
        for (const [network, config] of Object.entries(allNetworkIndexer)) {
            ...
            let archiveNodeLatestBlock = await getDataToMonitorStatus.getLatestBlockNumber(archiveNode, network)
            let externalRpcNodeLatestBlock = await getDataToMonitorStatus.getLatestBlockNumber(externalRpcNode, network)
            let subgraphData = await getDataToMonitorStatus.getSubgraphData(graphIndexerNode)
    ...
    getErrorMessage(archiveNodeLatestBlock, externalRpcNodeLatestBlock, subgraphData, currentNetwork) {
        ...
        let subgraphErrors = getDataToMonitorStatus.getSubgraphError(subgraphData, archiveNodeLatestBlock, currentNetwork)
